...
const Form = () => {
  ...
  const [referral, setReferral] = useState(id.toUpperCase());
 
  const [emailcolor, setemailColor] = useState("");
  const [linkedincolor, setlinkedinColor] = useState("");
  let helperText=""
  if (!id.toUpperCase().includes("HTMHA")) {
    id = "No Referral Applied";
    helperText="Referrals are given by existing Hack Ambassadors. Find them on our LinkedIn"

  } else {
    if (id.toUpperCase().includes("HTMHA")) {
      helperText=`Referral Applied: ${id}`;
    }
  }
  const data = {
    name: name,
  ...
  const sendResponse = async (data) => {
    ...
    if (ticketGenerated) {
      ...
      setSuccess(true);
      setMessage(
        "We got your ambassador application. You can check your status, using this hack ambassador ticket id:"
      );
    }
      ...
      {!loading && !success && (
        <form noValidate autoComplete="off">
          ...
          <Button
            ...
            onClick={(e) => {
              if (
                ValidationService(1, name) &&
                ValidationService(0, email) &&
                college &&
                linkedin
              ) {
                sendResponse(data);
